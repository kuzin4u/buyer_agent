...
class Pipeline:
    ...
    def run(self, receipts):
        ...
        out.items = [self._item(canon, r, it, out.fractional)
                     for canon, r in food_receipts for it in r.items]
        return out

    def _item(self, venue, receipt, raw_item, fractional):
        rules = self.rules
        name = N.clean(rules, raw_item.name)
        match = N.fold(rules, name)
        item = NormalizedItem(
            dt=receipt.dt, venue=venue, raw=raw_item.name, name=name, match=match,
            qty=raw_item.qty, price=raw_item.price, amount=raw_item.total)
...
def to_history(run, principal="owner"):
    ...
    return History(principal=principal, events=events, source="receipts_fns")
