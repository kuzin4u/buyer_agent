...
def render(run):
    ...
    # Знаменатель всех долей ниже печатается явно: одна и та же метрика при
    # двух способах счёта даёт два числа, и спорить об этом каждый раз заново
    # дороже, чем один раз написать (docs/DECISIONS.md Р-13).
    p(f"  анализируется: {cov.analysable} позиций, "
      f"{_money(f'{cov.money:,.0f}')} ₽ — знаменатель всех долей ниже")
    ...
    p(f"  не отнесено к группе: {_money(f'{cov.unmatched_total:,.0f}')} ₽ "
      f"из {_money(f'{cov.money:,.0f}')} ₽ ({cov.unmatched_share:.1%} от анализируемых)")
