...
Браузерный класс помечен `@slow`: три запуска Chrome стоят несколько секунд, а
...
class ChartMarkupTest(unittest.TestCase):
    """Быстрая стража того же самого — по исходникам, без браузера.

    Браузерный класс точнее, но стоит секунды и требует Chrome. Эти проверки
    держат те же три условия задаром и поэтому идут в каждом прогоне.
    """
